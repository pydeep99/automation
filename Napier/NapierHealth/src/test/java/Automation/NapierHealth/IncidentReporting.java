...
public class IncidentReporting extends base {
	...
	public void credentialPage(String Username,String Password, String date) throws IOException
	{
		...
		ip.getIncidentCategory().sendKeys("Medication Errors");
		ip.getIncidentCategory().sendKeys(Keys.ARROW_DOWN,Keys.ENTER);
		ip.getIncidentSubCategory().sendKeys("other");
		ip.getIncidentSubCategory().sendKeys(Keys.ARROW_DOWN,Keys.ENTER);
		ip.getAddRow().click();
		ip.getAddRow().sendKeys(Keys.TAB);
		js.executeScript("window.scrollBy(0,document.body.scrollHeight)");
		
		js.executeScript("arguments[0].scrollIntoView();", ip.getSave());
		ip.getSave().click();
